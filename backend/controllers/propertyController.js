...
const propertyController = {
    ...
    updateProperty: async (req, res) => {
        try {
            ...
            // ✅ is_active 전용 처리
            const isStatusOnly =
                Object.keys(propertyData).every(k => ['is_active', 'thumbnailIndex'].includes(k)) &&
                'is_active' in propertyData;

            if (isStatusOnly) {
                const property = await propertyModel.getPropertyByIdAllowInactive(id);
                if (!property) {
                    return res.status(404).json({ message: 'Property not found' });
                }
                ...
                await propertyModel.updateProperty(id, { is_active: propertyData.is_active });

                const complete = await propertyModel.getPropertyByIdAllowInactive(id);
            ...
            await propertyModel.updateProperty(id, propertyData);
