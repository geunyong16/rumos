...
const agentController = {
    ...
    getAgentProfile: async (req, res) => {
        try {
            ...
            res.status(200).json({ agentProfile: agent });
        } catch (error) {
    ...
    getAgentProperties: async (req, res) => {
        try {
             ...
             // (필요에 따라) agent_id 를 먼저 구한 다음 매물을 조회할 수도 있음
             // 예) const agent = await agentModel.getAgentByUserId(userId);
             //     const properties = await agentModel.getPropertiesByAgentId(agent.agent_id);
 
             const properties = await agentModel.getPropertiesByUserId(userId);
             return res.status(200).json({ properties });
         } catch (error) {
             console.error('Error getting agent properties:', error);
             return res.status(500).json({ message: 'Error getting agent properties' });
         }
     },
